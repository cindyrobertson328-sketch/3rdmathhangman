...
const Modal: React.FC<ModalProps> = ({ status, word, onPlayAgain, onExit }) => {
  ...
  const actionLabel = isWinner ? 'Next Word ➡️' : 'Try Another Word 🔄';

  return (
    <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[100] p-4 animate-fade">
      <div className={`text-center p-8 rounded-2xl shadow-2xl w-full max-w-md animate-slide-up ${isWinner ? 'bg-green-100' : 'bg-red-100'}`}>
        <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${isWinner ? 'text-green-800' : 'text-red-800'}`}>{title}</h2>
        
        <div className={`text-lg md:text-xl mb-6 ${isWinner ? 'text-green-700' : 'text-red-700'}`}>
            <p className="mb-2">{isWinner ? 'You guessed the word!' : 'You ran out of guesses.'}</p>
            <p>The word was: <span className="font-bold uppercase tracking-wide">{word}</span></p>
        </div>
